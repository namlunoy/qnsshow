...
public class HttpWork {
	...
	/**
	 * Thực hiện tạo header để lấy được content chuẩn về
	 * Sau đó bóc tách luôn trong hàm này
	 * @param p
	 * @return
	 */
	public HeaderParam DoRequest(HeaderParam p)
	{
		...
		//	System.out.println(html);
			...
			while(matchs.find())
				page = matchs.group();
			hp.theLastPage = Integer.parseInt(page.substring(6,page.indexOf("</span>")));
			System.out.println("Get page "+hp.theLastPage+" done!");
			
			
			
			//------------------------ Xử lý dữ liệu nhận được -------------------------
			CollectDataFromUrl.UpdateTheShowFromQnsDotVN(html);
